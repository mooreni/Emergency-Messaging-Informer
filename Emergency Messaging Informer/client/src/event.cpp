...
names_and_events parseEventsFile(std::string json_path)
{
    ...
    for (auto &event : data["events"])
    {
        ...
        int date_time = (int)event["date_time"];
...
std::string Event::toString() const {
    ...
    // Add user if set
    if (!eventOwnerUser.empty()) {
        output << "user:" << eventOwnerUser << "\n";
    }

    // Add other details
    output << "city:" << city << "\n";
    output << "event_name:" << name << "\n";
    output << "date_time:" << date_time << "\n";
...
std::string Event::epoch_to_date(int epoch_seconds) const{
    std::time_t time = epoch_seconds;
    std::tm* tm_ptr = std::localtime(&time);

    std::ostringstream oss;
    oss << std::put_time(tm_ptr, "%d/%m/%y %H:%M");  // Format: DD/MM/YY HH:MM
    return oss.str();
}

std::string Event::toStringSummary() const {
    std::string result;
    result += "city : " + city + "\n";

    // Convert epoch to date
    result += "date time : " + epoch_to_date(date_time) + "\n";

    result += "event name : " + name + "\n";

    // Generate the summary
    std::string short_description = description.substr(12, 27);
    if (description.length() > 39) {
        short_description += "...";
    }
    result += "summary : " + short_description + "\n";
